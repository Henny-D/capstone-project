...
const SynonymMode = () => {
  ...
  const getRandomWord = synonymModeStore((state) => state.getRandomWord);
  ...
  const fetchSynonyms = async (word) => {
    try {
      ...
      if (!response.ok) throw new Error ("API is not responding. Please try again.");
        ...
        const synonymList =data[0]?.meanings?.flatMap((meaning) => meaning.synonyms) || [];

        if (synonymList.length > 0) return synonymList;
        handleError("Sorry, no Synonyms found for this word. Play again.");
        return [];
        }  catch (error) {
  ...
  const loadNewWord = async () => {
    ...
    setSynonyms(await fetchSynonyms(newWord.word));
  };
  ...
  const checkAnswer = () => {
    if (synonyms.includes(input.trim().toLowerCase())) {
  ...
  return (
    <div className='flex flex-col justify-center items-center min-h-screen bg-yellow-500 p-6 font-poppins'>
      ...
      <div  className='mt-10 text-center font-poppins max-w-md w-full sm:w-1/2 md:w-1/3 lg:w-1/4'>
        ...
        <div>
          <h2 className='font-poppins text-5xl font-bold text-zinc-800 mb-4'>Synonym</h2>
          <SynonymDisplay word={word} />
